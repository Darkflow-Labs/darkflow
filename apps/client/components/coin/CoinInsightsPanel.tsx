      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <Button
            ...
            aria-label={pinned ? "Unpin token from toolbar" : "Pin token to toolbar"}
            onClick={onTogglePin}
          >
...
export const CoinInsightsPanel = ({
  ...
  const pinned = isTokenPinned(selectedSymbol);
  const handleTogglePin = () => {
    if (pinned) {
      unpinToken(selectedSymbol);
      return;
    }
    pinToken({ symbol: selectedSymbol, label: coinInsightsLabel });
  };
  ...
  const desktopAside = (
    <motion.aside
      ...
      style={{
        ...
        width: coinWidthPx,
        ...(floatingLeftPx != null
          ? { left: floatingLeftPx, right: "auto" }
          : dockAdjacent
            ? { right: dockRightPx }
            : {}),
        ...(floatingLeftPx == null && !dockAdjacent && hubCoinLeftPx != null
          ? { left: hubCoinLeftPx, right: "auto" }
          : {}),
      }}
    >
      <CoinInsightsBody
